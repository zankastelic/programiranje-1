...
let rec list_of_tree1 = function
  | Empty -> []
  | Node (lt, x, rt) -> 
    let left_list = list_of_tree1 lt in
    let right_list = list_of_tree1 rt in 
...
let is_bst tree =
  ...
  (*Zadoščati moramo low_bound < x < high _bound *)
  | Empty -> true
  | Node (lt, x, rt) ->
      let this_ok = 
...
(*let is_bst tree = 
  ...
  | Empty -> true
  | Node (lt, x, rt) -> 
      let this_ok = low_bound < x < high_bound in 
      let left_ok = is_bst' low_bound (Some x) lt in
      let right_ok = is_bst' (Some x) high_bound rt in (*ko vse preverimo, združimo*)
      this_ok && left_ok && right_ok
  in 
  is_bst' None None tree
*)
